# app.py (Güncellenmiş)
from flask import Flask, request, render_template  # render_template'i içe aktar
import json
...
@app.route("/")
def index():
    # templates klasörü altındaki index.html dosyasını işleyerek döndürür
...
def doyoumean():
    ...
    for key in veri.keys():
        ...
        if benzerlik > 50:
            return f"{key}, ana listedeki {target} ile benziyor. Sonuc: {veri[key]} "
        eslesme = True
        break
    ...
    if not eslesme:
        for key1 in veri1.keys():
            ...
            if benzerlik1 > 40:
                return f"yedek listedeki {key1}, {target} ile benziyor. Sonuc: {veri1[key1]}"
    ...
    if not eslesme:
        return f"{domain}, iki listede de bulunmadigi icin internetten aratilarak bulundu."
        veri1[target] = domain
